...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user:**
   ...
   Otherwise, inspect the selected planning root's `openspec/config.yaml`. When
   it declares a project-local `schema`, pass that value explicitly because CLI
   versions may still report or scaffold `spec-driven` as their built-in default.
   Omit `--schema` only when the project does not declare one.

3. **Create the change directory**

   Choose one schema form below. If a registered store is selected, append `--store "<store-id>"` to that command and each later OpenSpec command shown below that accepts `--store`.

   Using the CLI built-in default when no project schema is declared:
   ```bash
   openspec new change "<name>"
   ```

   Using a project-declared or explicitly requested schema:
   ```bash
...
4. **Get the artifact build order**
   ...
   **Approval preflight for capability-based PRD workflows:** When the graph
   contains `prd`, `prd-capabilities`, and `product-approval`, do not rely on the
   approval artifact's file-existence status. Before creating or updating any
   engineering artifact, read the current product-approval instructions and all
   PRD-set files from `artifactPaths`. Verify that the `prd.md` index exactly
...
5. **Create every artifact in the required set**
   ...
   **Capability-PRD human gates override automatic continuation.** When the graph
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      ...
      - Perform every approval or traceability preflight in `instruction` before
        writing. A completed approval file is not proof that its digest is current
...
- Never create proposal, specs, design, or tasks from a stale PRD-set approval.
  For capability-based PRDs, references must use
  `<change-name>#<capability-path>/FR-001` (or the corresponding NFR form), and
  every Must requirement across all indexed capability PRDs must map to an
  OpenSpec requirement
- `skip_specs: true` is allowed only when there is no spec-level behavior change
  and the complete PRD set contains zero Must requirements
