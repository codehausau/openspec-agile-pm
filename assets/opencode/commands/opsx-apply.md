...
2. **Check status to understand the schema**
   ...
   **Run approval preflights before handling any apply state.** When the graph
   contains `prd`, `prd-capabilities`, and `product-approval`, read their concrete
   paths from `artifactPaths` and the current product-approval artifact
   instructions. Verify the `prd.md` index exactly matches all capability files,
   recompute the deterministic PRD-set manifest digest and file count, and compare
   both with `product-approval.md`. If approval is missing or either value differs,
...
- Never implement against a missing or stale PRD-set approval; perform its
  preflight before interpreting blocked, ready, or all-done apply state
