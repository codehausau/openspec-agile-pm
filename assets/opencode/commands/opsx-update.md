...
2. **Get the change's artifacts**
   ...
   the capability-PRD approval preflight before reconciling engineering artifacts.
   Read the current product-approval instructions and concrete PRD-set files,
   verify the `prd.md` index exactly matches all capability files, and compare the
   ...
   `workflows/product-publication.md`, requiring format 3, exact candidate/MkDocs/plan
   approval, and matching full baseline. Publication of this increment is not a
   prerequisite to engineering; it happens at archive after delivery reconciliation.
...
4. **Read and reconcile**
   ...
   - If the revision changes `prd.md`, any indexed capability PRD, `product-docs/`, or `product-publication.yaml`,
     ...
     working approval, reconciles the increment and full proposed product set,
     and obtains fresh approval for archive-time publication. Do not edit published
     product pages or the live MkDocs configuration directly. Do not revise
...
5. **Confirm and apply, one artifact at a time**
   ...
   - Before every artifact edit, get that artifact's current rules, template, and
     instruction, not only for substantial rewrites:
     ```bash
...
- Never leave `product-approval.md` in place while changing any file in a
  capability-based PRD set. Never reconcile engineering artifacts against stale
  approval, even when status reports the approval artifact as done.
